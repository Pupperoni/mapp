...
Dir.foreach('./lib/seeds/accidents') do |item|
  ...
  csv.each do |row|
    if Location.exists?(location: row['place_of_occurance'])
      loc = Location.where(location: row['place_of_occurance']).first.id
    else
      t = Location.new
      t.location = row['place_of_occurance']
      t.save
      loc = t.id
    end
...
Dir.foreach('./lib/seeds/cargo-freight') do |item|
  ...
  csv.each do |row|
    if Location.exists?(location: row['airport'])
      loc = Location.where(location: row['airport']).first.id
    else
      t = Location.new
      t.location = row['airport']
      t.save
      loc = t.id
      t = Airport.new
      t.location_id = loc
...
Dir.foreach('./lib/seeds/infrastructure') do |item|
  ...
  csv.each do |row|
    # if Location.exists?(location: row['project_location'])
    #   loc = Location.where(location: row['project_location']).first.id
    # else
    #   t = Location.new
    #   t.location = row['project_location']
    #   t.save
    #   loc = t.id
    #   t = Project.new
